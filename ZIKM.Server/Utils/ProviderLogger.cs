...
namespace ZIKM.Server.Utils {
    class ProviderLogger {
        ...
        public void ToLogProvider(ReadOnlySpan<byte> data) {
            using StreamWriter stream = new StreamWriter(Path.Combine(logProviderPath, $"{DateTime.Today.ToString("dd-MM-yyyy")}.log"), true);
            ...
            stream.WriteLine($"{DateTime.Now.ToString()} : {string.Join(";", msEncrypt.ToArray())}");
        }
    }
}
